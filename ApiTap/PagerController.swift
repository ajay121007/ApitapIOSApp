...
open class PagerController: BaseViewController, UIPageViewControllerDataSource, UIPageViewControllerDelegate {

	// MARK: - public properties
    var delegateStore: storeDetailsInfo?
     var vc = BaseViewController()
	open var contentViewBackgroundColor: UIColor = UIColor.white
	open var indicatorColor: UIColor = UIColor.red
    open var indicatorBackGroundColor: UIColor = UIColor.yellow
	...
	open func setupPager(tabNames: [String], tabControllers: [UIViewController],tabImg: [String]) {
        var indexFoRimage = 0
		let tabViews = tabNames.map { title -> UIView in
			let label = UILabel()
			label.text = title
			...
			label.sizeToFit()
            var img = UIImage()
            img = UIImage(named: tabImg[indexFoRimage])!
            indexFoRimage = indexFoRimage+1
            
            label.attributedText = atributedLabel(str: title, img: img)
	...
	func defaultSetup() {
		...
		let index: Int = self.startFromSecondTab ? 1 : 0
         if TestClass2.doubleVarBool == true{
            
            self.selectTabAtIndex(TestClass.doubleVar, swipe: true)
         }else{
	...
	func updateSelectedTab(_ index: Int) {
		...
		if let label = tab?.subviews.first as? UILabel {
            ...
            tab?.backgroundColor = indicatorBackGroundColor
		}
	}
	...
	func viewControllerAtIndex(_ index: Int) -> UIViewController? {
		...
		if (self.contents[index] as UIViewController?) == nil {
          ...
          //viewController.didMove(toParentViewController: self.pageViewController.view)
		}
		return self.contents[index]
	}
    func changeBackgroundColor(_ storeName: String, _ imageUrl: String) {
        
        delegateStore?.getStoreDetails(storeName, imageUrl)
        //self.navigationController?.popViewController(animated: true)
        // let detail = storyboard?.instantiateViewController(withIdentifier: "HomeVC1") as? HomeVC
        
        // detail?.storeNameStr = storeName
        
        //  detail?.imageStr = imageUrl
        
        // detail?.itemsTblView.reloadData()
	...
	open override func scrollViewWillBeginZooming(_ scrollView: UIScrollView, with view: UIView?) {
		if self.actualDelegate != nil {
	...
	open override func scrollViewDidEndZooming(_ scrollView: UIScrollView, with view: UIView?, atScale scale: CGFloat) {
		if self.actualDelegate != nil {
